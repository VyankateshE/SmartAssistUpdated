...
const getCallAnalytics = async (req, res) => {
  try {
    ...
    const userID = req.query.user_id ? req.query.user_id : req.userId;
    ...
    for (const {
      ...
      const isLead = leads.some(({ mobile: leadMobile }) => {
        if (!mobile || !leadMobile) return false;
        return mobile.includes(leadMobile) || leadMobile.includes(mobile);
      });
...
module.exports = { getCallAnalytics };
